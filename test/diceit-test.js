...
describe('DiceIt', () => {
  it('Should deploy and run', async() => {
    ...
    expect(receipt.events[0].event).to.equal('GameStageChange')
    expect(receipt.events[0].args.index).to.equal(0)
    expect(receipt.events[0].args._stage).to.equal(0)
    ...
    expect(receipt.events[0].event).to.equal('GameStageChange')
    expect(receipt.events[0].args.index).to.equal(0)
    expect(receipt.events[0].args._stage).to.equal(1)
    expect(receipt.events[0].args._from).to.equal(challenger.address)
